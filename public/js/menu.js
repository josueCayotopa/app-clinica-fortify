...
var Menu = /*#__PURE__*/
  function () {
    ...
    _createClass(Menu, [{
      ...
      value: function _bindEvents() {
        ...
        this._evntElClick = function (e) {
          ...
          if (e.target.closest('ul') && e.target.closest('ul').classList.contains('menu-inner')) {
            ...
            if (menuItem)
              _this._topParent = menuItem.childNodes[0];
          }
        ...
        this._evntWindowResize = function () {
          ...
          if (!_this._horizontal && !horizontalMenuTemplate)
            _this.manageScroll();
        }
          ;
        ...
        Menu._promisify(this._onOpen, this, item, toggleLink, Menu._findMenu(item)).then(function () {
          if (!_this2._horizontal || !Menu._isRoot(item)) {
            if (_this2._animate && !_this2._horizontal) {
              ...
              if (_this2._accordion)
                _this2._closeOther(item, closeChildren);
      ...
      key: "childOf",
      value: function childOf(/* child node */
        c, /* parent node */
        p) {
        // returns boolean
        if (c.parentNode) {
      ...
      value: function _findParent(el, cls) {
        ...
        if (!el && throwError)
          throw new Error("Cannot find `.".concat(cls, "` parent element"));
        return el;
      }
    }, {
      key: "_findChild",
        ...
        for (var i = 0, l = items.length; i < l; i++) {
          if (items[i].classList) {
            ...
            for (var j = 0; j < cls.length; j++) {
              if (items[i].classList.contains(cls[j]))
                passed += 1;
            }

            if (cls.length === passed)
              found.push(items[i]);
          }
        }
      ...
      value: function _getItem(el, toggle) {
        ...
        if (el.classList.contains('menu-item')) {
          if (Menu._findChild(el, [selector]).length)
            item = el;
      ...
      value: function _getLink(el, toggle) {
        ...
        if (!found.length)
          throw new Error("`".concat(selector, "` element not found."));
        return found[0];
      }
    }, {
