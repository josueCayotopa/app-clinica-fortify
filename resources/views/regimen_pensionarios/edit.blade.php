...
<div class="card-body mt-5 mb-1">
    <div class="tab-pane fade show active" id="datos-empresa">
        <form id="form-datos-empresa" method="POST" action="{{ route('descuentos_pensiones.update', $descuento->id) }}"
            autocomplete="off">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="form-group">
                        <label for="regimen_id">Regimen Pensionario</label>
                        <select name="regimen_id" id="regimen_id" class="form-control">
                            ...
                            @foreach ($regimenes as $regimen)
                                <option value="{{ $regimen->id }}"
                                    {{ $descuento->regimen_id == $regimen->id ? 'selected' : '' }}>
                                    {{ $regimen->descripcion }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <select name="descuento_id" class="form-control">
                                ...
                                @foreach ($descuentos as $descuentoOption)
                                    <option value="{{ $descuentoOption->id }}"
                                        {{ $descuento->descuento_id == $descuentoOption->id ? 'selected' : '' }}>
                                        {{ $descuentoOption->descripcion }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="form-group">
                        ...
                        <select name="tipo_comision" class="form-control" id="tipo_comision">
                            <option value="FLUJO" {{ $descuento->tipo_comision == 'FLUJO' ? 'selected' : '' }}>
                                FLUJO</option>
                            <option value="MIXTA" {{ $descuento->tipo_comision == 'MIXTA' ? 'selected' : '' }}>
                                MIXTA</option>
                        </select>
                        @error('tipo_comision')
                            <span class="text-danger">{{ $message }}</span>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="form-group">
                        ...
                        @php
                            // Convertir la fecha de la base de datos al formato YYYY-MM
                            $fechaFormateada = date('Y-m', strtotime($descuento->fecha));
                        @endphp
                ...
                <div class="col-md-6 mb-3">
                    <div class="form-group">
                        ...
                        <input type="number" step="0.01" class="form-control" id="porcentaje" name="porcentaje"
                            placeholder="Porcentaje" value="{{ $descuento->porcentaje }}">
                        @error('porcentaje')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="form-group">
                        ...
                        <input type="number" step="0.01" class="form-control" id="importe_tope" name="importe_tope"
                            placeholder="Importe Tope" value="{{ $descuento->importe_tope }}">
                        @error('importe_tope')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
